...
#'   `metadata.json` file. A flat metadata tibble is refused: it is what this
#'   function returns, and every other metadata function takes it as it is.
#'
...
metadata_studies <- function(metadata, call = rlang::caller_env()) {
  ...
  if (is.data.frame(metadata)) {
    cli::cli_abort(
      c(
        "{.arg metadata} is already a flat metadata tibble.",
        "i" = "{.fn jatos_flatten_metadata} takes the parsed server answer; the tibble comes from {.fn jatos_results_metadata} or {.fn jatos_read_metadata} and needs no flattening."
      ),
      call = call,
